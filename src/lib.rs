...
mod tests {
    ...
    fn setup_test_db(test_name: &str) -> Result<(String, Schema, Arc<Mutex<StorageManager>>, TransactionManager), DbError> {
    ...
    fn test_join() {
        ...
        let join_query = Query::Join {
            ...
            columns: vec!["Employees.Name".to_string(), "Departments.DeptName".to_string()],
            condition: None,
        };
        let mut tx = tx_manager.begin_transaction();
    ...
    fn test_transaction_commit_rollback() {
        let (data_dir, _schema, _storage, mut tx_manager) = setup_test_db("tx_commit_rollback").unwrap();
        let create_query = Query::CreateTable {
    ...
    fn test_error_handling() {
        let (data_dir, _schema, _storage, mut tx_manager) = setup_test_db("error_handling").unwrap();
        let create_query = Query::CreateTable {
            table: "Test".to_string(),
            columns: vec![
                ("ID".to_string(), DataType::Int32),
        ...
        let insert_query = Query::Insert {
            table: "Test".to_string(),
            values: vec![Value::String("Invalid".to_string()), Value::String("Test".to_string())],
        };
        let mut tx = tx_manager.begin_transaction();
        tx.add_query(insert_query);
